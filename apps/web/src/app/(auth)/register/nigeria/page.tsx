...
export default function NigeriaRegisterPage() {
  const router = useRouter();
  const setAuth = useAuthStore(s => s.setAuth);
  const [form, setForm] = useState({ name: '', email: '', phone: '', password: '', password_confirmation: '', country: 'NG', role: 'client', business_name: '' });
    ...
    <div className="min-h-screen bg-[#080C12] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white mt-2">Join TheOnlineYard Nigeria</h1>
          <p className="text-[#7088A8] text-sm mt-1">Rent and buy vehicles across Nigeria</p>
        </div>
        <form onSubmit={submit} className="bg-[#141D2B] rounded-2xl p-8 space-y-4">
          ...
          <div><label className="text-[#7088A8] text-xs">Phone (NGN)</label><div className="flex mt-1"><span className="bg-[#7088A8]/10 border border-r-0 border-[#7088A8]/20 rounded-l-lg px-3 py-2.5 text-[#7088A8] text-sm">+234</span><input value={form.phone} onChange={e => setForm(f=>({...f, phone: e.target.value}))} placeholder="8012345678" className="flex-1 bg-[#080C12] border border-[#7088A8]/20 rounded-r-lg px-3 py-2.5 text-white"/></div></div>
          ...
          <div className="bg-[#080C12] rounded-lg p-3 text-xs text-[#7088A8]">💳 Payments processed via <span className="text-white">Paystack</span> (NGN cards, bank transfer, USSD)</div>
