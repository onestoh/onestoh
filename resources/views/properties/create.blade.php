@extends('layouts.app')
@section('title', 'List a Property — EstateYard')
...
<style>
  ...
  .amenity-item input[type=checkbox]:checked + label { color: var(--gold); }
...
<div class="property-form-page">
  ...
  <div class="page-breadcrumb">
    <a href="{{ url('/dashboard') }}">Dashboard</a> &rsaquo; List New Property
  </div>
  <div class="page-heading">🏘️ List a New Property</div>
  ...
  <form action="{{ url('/properties') }}" method="POST" enctype="multipart/form-data" data-validate>
    @csrf
    ...
    <div class="form-section">
      ...
      <div class="form-group" style="margin-bottom:16px;">
        ...
        <input type="text" name="title" class="form-control" placeholder="e.g. Modern 4-Bed Villa in Karen" value="{{ old('title') }}" required maxlength="255">
      </div>
      <div class="form-grid-2">
        ...
        <div class="form-group">
          ...
          <select name="listing_type" class="form-control" required>
            ...
            <option value="{{ $lt }}" {{ old('listing_type') === $lt ? 'selected' : '' }}>{{ ucwords(str_replace('_',' ',$lt)) }}</option>
            @endforeach
          </select>
        </div>
      </div>
    ...
    <div class="form-section">
      ...
      <div class="form-group">
        ...
        <textarea name="description" class="form-control" rows="6" placeholder="Describe the property in detail — layout, condition, unique features, nearby amenities..." required style="resize:vertical;">{{ old('description') }}</textarea>
      </div>
    </div>
    ...
    <div class="form-section">
      ...
      <div class="form-grid-2">
        ...
        <div class="form-group">
          ...
          <input type="text" name="constituency" class="form-control" placeholder="e.g. Westlands" value="{{ old('constituency') }}">
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Specific Location / Address <span style="color:var(--red);">*</span></label>
        <input type="text" name="location" class="form-control" placeholder="e.g. Karen Road, off Langata Road" value="{{ old('location') }}" required>
      </div>
    </div>
    ...
    <div class="form-section">
      ...
      <div class="form-grid-3">
        <div class="form-group">
          ...
          <input type="number" name="bedrooms" class="form-control" placeholder="0" min="0" max="50" value="{{ old('bedrooms') }}">
        </div>
        <div class="form-group">
          <label class="form-label">Bathrooms</label>
          <input type="number" name="bathrooms" class="form-control" placeholder="0" min="0" max="50" value="{{ old('bathrooms') }}">
        </div>
        <div class="form-group">
          <label class="form-label">Area (sq ft)</label>
          <input type="number" name="area_sqft" class="form-control" placeholder="e.g. 2500" min="0" step="0.01" value="{{ old('area_sqft') }}">
        </div>
        <div class="form-group">
          <label class="form-label">Year Built</label>
          <input type="number" name="year_built" class="form-control" placeholder="e.g. 2018" min="1900" max="2100" value="{{ old('year_built') }}">
        </div>
        <div class="form-group">
          <label class="form-label">Number of Floors</label>
          <input type="number" name="floors" class="form-control" placeholder="e.g. 2" min="1" value="{{ old('floors') }}">
        </div>
      </div>
    </div>
    ...
    <div class="form-section">
      ...
      <div class="form-grid-2">
        <div class="form-group">
          ...
          <input type="number" name="price" class="form-control" placeholder="e.g. 12000000" min="1" step="0.01" value="{{ old('price') }}" required>
        </div>
        <div class="form-group">
          ...
          <select name="price_period" class="form-control">
            ...
            <option value="{{ $pp }}" {{ old('price_period') === $pp ? 'selected' : '' }}>{{ ucfirst($pp) }}</option>
            @endforeach
          </select>
        </div>
      </div>
    ...
    {{-- IMAGES --}}
    <div class="form-section">
      <div class="form-section-title">📷 Property Images <span style="font-size:13px; font-weight:400; color:var(--muted);">(up to 10 images, 5MB each)</span></div>
      <div class="image-upload-zone" onclick="document.getElementById('imageInput').click();" id="uploadZone">
        <div style="font-size:36px; margin-bottom:12px;">📷</div>
        <div style="font-size:15px; color:var(--white); font-weight:600; margin-bottom:6px;">Click to upload images</div>
        <div style="font-size:13px; color:var(--muted);">JPG, PNG, WebP · Max 5MB per image · Up to 10 images</div>
      </div>
